...
def project_dct(param, *, chunk_size, transpose=False, to_bulk=False):
    ...
    einsum_expr = "".join(
        ...
        + ["->"] +
        ([base_letters + dct_result_letters] if to_bulk else [
            b + r for b, r in zip(base_letters, dct_result_letters)
        ])
    )
    unflattened_result = jnp.einsum(einsum_expr, param, *(mat for _ in dct_letters))
    return unflattened_result.reshape(*unflattened_result.shape[:ndim], -1) \
        if to_bulk else unflattened_result.reshape(orig_shape)
...
def extract_dct(param, *, config: ProjectConfig):
    def map_fn(x):
        bulked = project_dct(x, chunk_size=config.chunk_size, to_bulk=True)
...
class DemoState(eqx.Module):
    ...
    mu: base.Updates
    last_q: base.Updates
    last_unprojected_q: base.Updates
    config: ProjectConfig = eqx.field(static=True)


def scale_by_demo(b1=0.999, eps=1e-8, config: ProjectConfig = ProjectConfig(chunk_size=8, k=2)) -> base.GradientTransformation:
    def init_fn(params):
        ...
        return DemoState(
            ...
            last_unprojected_q=jax.tree.map(lambda x: jnp.zeros_like(x), params),
            config=config,
        )
    ...
    def update_fn(updates, state, params=None):
        ...
        unprojected_current_q = reconstruct_dct(q, config=state.config)
        mu = jax.tree.map(lambda x, y: x - y, mu, unprojected_current_q)
        update = state.last_unprojected_q
        update = jax.tree.map(lambda x: jnp.sign(x), update)
        return update, replace(
            state,
            mu=mu,
            last_q=q,
            last_unprojected_q=unprojected_current_q,
            count=state.count + 1,
        )
